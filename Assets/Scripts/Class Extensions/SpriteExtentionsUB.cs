...
public static class SpriteExtentionsUB
{
    ...
    /// <summary>
    /// Get only piece of texture in which is sprite. 
    /// This method can be used for displaying sprite as preview in editor windows.
    /// </summary>
    public static Texture2D GetTextureForSpritePreview(this Sprite sprite)
    {
        Rect spriteRect = sprite.rect;
        Texture2D textureInWhichIsSprite = sprite.texture;

        if (DoesTextureHaveSameDimensionsAsSprite(spriteRect, textureInWhichIsSprite))
        {
            Texture2D newTexture = new Texture2D(
                (int)spriteRect.width,
                (int)spriteRect.height);

            Color[] spritePixels = textureInWhichIsSprite.GetPixels(
                                                         (int)spriteRect.x,
                                                         (int)spriteRect.y,
                                                         (int)spriteRect.width,
                                                         (int)spriteRect.height);
            ...
            return textureInWhichIsSprite;
    }

    private static bool DoesTextureHaveSameDimensionsAsSprite(Rect spriteRect, Texture2D textureInWhichIsSprite)
    {
        return spriteRect.width != textureInWhichIsSprite.width || spriteRect.height != textureInWhichIsSprite.height;
    }
