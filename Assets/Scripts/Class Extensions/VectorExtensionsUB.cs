...
{
   public static  float DistanceToLine(this Vector3 point, Vector3 beginOfLine, Vector3 endOfLine)
    {
        Vector3 direction = endOfLine - beginOfLine;
        float magnitude = direction.magnitude;
        direction.Normalize();
        float dotProduct = Vector3.Dot(direction, point - beginOfLine);
        dotProduct = Mathf.Clamp(dotProduct, 0f, magnitude);
        return Vector3.Magnitude(beginOfLine + direction * dotProduct - point);
    }
